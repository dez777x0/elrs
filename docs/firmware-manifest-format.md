...
## Мультивыбор (.bin + .json за один раз)

В основном поле выбора прошивки можно отметить **несколько файлов** (Ctrl/⌘ в диалоге или несколько файлов при перетаскивании):

- Ровно один **`.bin`** или один **`.zip`** как прошивка; опционально один **`.json`** как sidecar.
- Если sidecar один и его имя совпадает с **`имя_прошивки.bin.json`** или **`имя_прошивки.json`**, он выбирается предпочтительно при нескольких `.json`.
- **`.zip` и `.bin` в одном выборе** не допускаются (неоднозначность).
- Несколько `.bin` или несколько `.zip` — ошибка с подсказкой.
