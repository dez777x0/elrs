...
Файлы **`.bin.gz`** (gzip) обрабатываются как одиночный образ: после распаковки применяются те же эвристики имени, что и для соответствующего **`.bin`** (имя без суффикса `.gz`).
...
- Ровно один **`.bin`**, **`.bin.gz`** или один **`.zip`** как прошивка; опционально один **`.json`** как sidecar.
