...
| Надёжность таймингов | `readLinesForMs` (Betaflight/INAV) завершается по **короткой тишине** при пустом буфере незавершённой строки (`idleFlushMs` ≈ 80 ms), не дожидаясь полного окна; при «рваной» доставке буфер не пуст — ждём до `totalMs`. |
